...
async function seedEntities() {
    ...
    for (let i = 0; i < entities.length; i += 25) {
        ...
        await dynamodb_1.docClient.send(new lib_dynamodb_1.BatchWriteCommand({
            RequestItems: {
                [dynamodb_1.TABLE_NAMES.ENTITIES]: requests,
            },
        }));
...
async function seedInitialPrices() {
    ...
    for (const entity of ENTITIES) {
        ...
        await dynamodb_1.docClient.send(new lib_dynamodb_1.PutCommand({
            TableName: dynamodb_1.TABLE_NAMES.PRICE_HISTORY,
            Item: priceHistory,
        }));
    }
